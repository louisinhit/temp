...
def gen_bfly_float16(args):
    ...
    np_input = Fxp(np_input, signed=True, n_word=int(16), n_frac=1)
    print (np_input)
    
    np.savetxt(path+'/input_bfly.txt', np_input.bin(), delimiter='\n', fmt='%s')
    ...
    for i in range(len(intern_results)):
        ...
        #######################
        x = torch.squeeze(weights[i]).cpu().detach().numpy()
        x = Fxp(x, signed=True, n_word=int(16), n_frac=1)
        np.savetxt(path+'/weight_stage'+str(i)+'.txt', x.bin(), delimiter='\n', fmt='%s')
